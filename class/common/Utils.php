...
class Utils
{
    ...
    public static function alert($content, $class)
    {
        ...
        return '<div class="alert ' . $class . ' alert-dismissible fade show" role="alert"><span class="text-semibold">' . $content . '</span><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>' . "\n";
    }
    ...
    public static function alertCard($title, $classname, $header_classname = '', $heading_elements = '', $content = '')
    {
        ...
        $card = static::startCard($title, $classname, $header_classname, $heading_elements, $has_body);
        $card .= $content;
        $card .= static::endCard($has_body);
    ...
    {
        if (is_null($value)) {
            return false; // Handle null values explicitly
        }
    ...
    public static function sanitize($var)
    {
        ...
        while (preg_match('`--`', $nouveauNom)) {
            $nouveauNom = str_replace('--', '-', $nouveauNom);
        }
    ...
    public static function getAttributes($attr)
    {
        if (empty($attr)) {
            ...
            $attr = preg_replace_callback('`(.),\s*`', [static::class, 'replaceCallback'], $attr);
