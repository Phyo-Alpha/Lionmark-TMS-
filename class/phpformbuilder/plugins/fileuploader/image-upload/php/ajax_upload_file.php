...
include_once '../../server/class.fileuploader.php';
...
if ($data['isSuccess']) {
    ...
    // Ensure the resized dimensions do not exceed the maximum values
    if ($resize_width > $width) {
        $resize_width = $width;
        $resize_height = $resize_width / $image_width * $image_height;
    }

    if ($resize_height > $height) {
        $resize_height = $height;
        $resize_width = $resize_height / $image_height * $image_width;
    }
