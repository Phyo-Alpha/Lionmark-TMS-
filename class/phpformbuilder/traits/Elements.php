...
trait Elements
{
     ...
     */
    public function addInput(string $type, string $name, string $value = '', string $label = '', string $attr = ''): self
    {
        if ($type == 'file') {
            $this->has_file = true;
        }
     ...
     *
     */
    public function addFileUpload(string $name, string $value = '', string $label = '', string $attr = '', array  $fileUpload_config = [], array $current_file = []): self
    {
        $this->has_file = true;
        $attr           = $this->getAttributes($attr); // returns linearised attributes (with ID)
        $array_values   = $this->getID($name, $attr); // if $attr contains no ID, field ID will be $name.
     ...
     * @return \phpformbuilder\Form
     */
    public function addOption(string $select_name, string $value, string $txt, string $group_name = '', string $attr = ''): self
    {
     ...
     * @return \phpformbuilder\Form
     */
    public function addSelect(string $select_name, string $label = '', string $attr = '', bool $displayGroupLabels = true): self
    {
        $attr          = $this->getAttributes($attr); // returns linearised attributes (with ID)
     ...
     * @return \phpformbuilder\Form
     */
    public function addCountrySelect(string $select_name, string $label = '', string $attr = '', array $user_options = []): self
    {
        ...
        $countries    = include_once $this->plugins_path . 'countries/country-list/country/cldr/' . $options['lang'] . '/country.php';
        $attr         = $this->getAttributes($attr); // returns linearised attributes (with ID)
        $array_values = $this->getID($select_name, $attr); // if $attr contains no ID, field ID will be $select_name.
        $id           = $array_values['id'];
        $attr         = $array_values['attributs']; // if $attr contains an ID, we remove it.
     ...
     * @return \phpformbuilder\Form
     */
    public function addTimeSelect(string $select_name, string $label = '', string $attr = '', array $user_options = []): self
    {
        /* define options*/
        $options = [
            'min'                => '00:00',
        ...
        for ($i = $min_minutes; $i <= $max_minutes; $i += $options['step']) {
            ...
            $current_minutes_pad = str_pad((string) $current_minutes, 2, '0', STR_PAD_LEFT);
            if ($options['format'] === '12hours') {
                if ($hours < 13) {
                    $opt_value = str_pad((string) $hours, 2, '0', STR_PAD_LEFT) . $options['value_separator'] . $current_minutes_pad . ' am';
                } else {
                    $opt_value = str_pad((string) ($hours - 12), 2, '0', STR_PAD_LEFT) . $options['value_separator'] . $current_minutes_pad . ' pm';
                }
            } else {
                $opt_value = str_pad((string) $hours, 2, '0', STR_PAD_LEFT) . $options['value_separator'] . $current_minutes_pad;
            }
     ...
     * @return \phpformbuilder\Form
     */
    public function addCheckbox(string $group_name, string $label, string $value, string $attr = ''): self
    {
        if ($this->framework == 'material') {
     ...
     * @return \phpformbuilder\Form
     */
    public function printCheckboxGroup(string $group_name, string $label = '', bool $inline = true, string $attr = ''): self
    {
        $start_wrapper = '';
        $end_wrapper   = '';
        $start_label   = '';
     ...
     * @return \phpformbuilder\Form
     */
    public function addRadio(string $group_name, string $label, string $value, string $attr = ''): self
    {
        if ($this->framework == 'material') {
     ...
     * @return \phpformbuilder\Form
     */
    public function printRadioGroup(string $group_name, string $label = '', bool $inline = true, string $attr = ''): self
    {
        $form_ID       = $this->form_ID;
        $start_wrapper = '';
        $end_wrapper   = '';
     ...
     * @return \phpformbuilder\Form
     */
    public function addBtn(string $type, string $name, string $value, string $text, string $attr = '', string $btnGroupName = ''): self
    {
