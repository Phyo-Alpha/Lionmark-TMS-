...
if ($show_external === true) {
    $is_last_insert_id_defined = false;
    foreach ($active_ext_cols as $key => $ext_col) {
        ...
        if (PDO_DRIVER === 'firebird' && $has_auto_increment_function && !$is_last_insert_id_defined) {
?>
                $<?php echo $generator->table; ?>_last_insert_ID = $next_<?php echo \strtolower($auto_increment_column_name); ?>;
<?php
            $is_last_insert_id_defined = true;
        } elseif (PDO_DRIVER === 'oci' && !$is_last_insert_id_defined) {
?>
...
<?php
            $is_last_insert_id_defined = true;
        } elseif (!$is_last_insert_id_defined) {
?>
