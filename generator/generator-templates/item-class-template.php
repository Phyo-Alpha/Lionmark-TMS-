...
<?php
    ...
    $pkv = '\'\''; // default value
    if (!empty($primary_keys_values)) {
        $pkv = implode(' . \'~\' . ', $primary_keys_values);
    }

?>
                );
                $this->pks[] = $primary_keys_array;
                $pk_concatenated_values = <?php echo $pkv ?>;
...
for ($i = 0; $i < $columns_count; $i++) {
    ...
    if ($generator->columns['select_from'][$i] == 'from_table' && $generator->columns['select_multiple'][$i] > 0) {
        ?>
                $json = Utils::decodeJson(<?php echo $columns['value'][$i]; ?>);
                if (is_array($json)) {
...
?>
                        $json = Utils::decodeJson($row-><?php echo $row_field; ?>);
                        if (is_array($json)) {
