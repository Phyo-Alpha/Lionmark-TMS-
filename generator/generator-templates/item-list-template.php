...
require_once GENERATOR_DIR . 'class/generator/TemplatesUtilities.php';
...
for ($i = 0; $i < $generator->columns_count; $i++) {
    if (!$generator->columns['skip'][$i]) {
        ...
        if ($generator->columns['sorting'][$i] && $generator->columns['nested'][$i] !== true) {
            ...
            ?>
                        <th<?php echo $footable_data; ?>>{{ object.fields.<?php echo $field_name; ?>|raw }}</th>
            <?php
        } // END else
    } // END if !skip
} // END for
