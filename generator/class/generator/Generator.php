...
class Generator
{
    ...
    */
    ...
    public function getTables()
    {
        ...
        if (empty($this->table) && isset($this->tables[0])) {
            ...
            */
            ...
            for ($i = 0; $i < $this->columns_count; $i++) {
                ...
                if ($field_type == 'file') {
                    ...
                    $special4 = '0';
                    $special7 = '0';
                    if (isset($_POST['cu_special_image_thumbnails_' . $column_name])) {
                        $special3 = $_POST['cu_special_image_thumbnails_' . $column_name];
                    }
                    if (isset($_POST['cu_special_image_editor_' . $column_name])) {
                    ...
                    if (isset($_POST['cu_special_image_crop_' . $column_name])) {
            ...
            if ($external_columns_count > 0) {
                foreach ($this->external_columns as $key => $ext_col) {
                    if ($ext_col['active'] === true && !empty($ext_col['relation']['intermediate_table'])) {
                        ...
                        if (isset($_POST['cu_ext_col_allow_in_forms-' . $key]) && (int) $_POST['cu_ext_col_allow_in_forms-' . $key] > 0) {
    ...
    * @return float|int
    */
    private function getMaxNumber(int $int, int $decimal = 0)
    {
        $units = '';
        for ($i = 0; $i < $int; $i++) {
            $units .= '9';
        ...
        return $decimal > 0 ? (float)($units . $dec) : (int)$units;
    }
    ...
    {
        $db = new DB(true);
        $this->is_connected = $db->isConnected();
        ...
        if ($this->is_connected === true) {
            ...
            $this->userMessage(FAILED_TO_CONNECT_DB, 'alert-danger has-icon', '', '', false);
        }
     ...
     * @return void
     */
    private function userMessage($title, $classname, $heading_elements = 'close', $content = '', $append = true)
    {
        if (!isset($_SESSION['msg']) || $append === false) {
            $_SESSION['msg'] = '';
        }
        if (!empty($content)) {
            // panel
