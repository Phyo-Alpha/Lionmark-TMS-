...
if (ENVIRONMENT !== 'development' && GENERATOR_LOCKED === true) {
    include_once GENERATOR_DIR . 'inc/protect.php';
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && Form::testToken('configuration-form') === true) {
    ...
    if ($validator->hasErrors()) {
        ...
        if ($userConf->datetimepickers_style == 'default' && isset($_POST['datetimepickers_lang'])) {
            $userConf->datetimepickers_lang = $_POST['datetimepickers_lang'];
        } elseif (isset($_POST['datetimepickers_material_lang'])) {
            $userConf->datetimepickers_lang = $_POST['datetimepickers_material_lang'];
        } else {
            $userConf->datetimepickers_lang = 'en_EN';
        }
        ...
        $userConf->enable_style_switching = true;
        if (!isset($_POST['enable_style_switching'])) {
            $userConf->enable_style_switching = false;
        }
        ...
        $userConf->php_errors_monitor_enabled = 0;
        if (isset($_POST['php_errors_monitor_enabled'])) {
            $userConf->php_errors_monitor_enabled = $_POST['php_errors_monitor_enabled'][0];
        }
        $userConf->php_errors_monitor_level = 1;
        if (isset($_POST['php_errors_monitor_level'])) {
            $userConf->php_errors_monitor_level = $_POST['php_errors_monitor_level'];
        }
        $userConf->php_errors_monitor_recipient_emails = '';
        if (isset($_POST['php_errors_monitor_recipient_emails'])) {
            $userConf->php_errors_monitor_recipient_emails = $_POST['php_errors_monitor_recipient_emails'];
        }
...
$form->endFieldset();
$form->startFieldset(PHP_ERRORS_MONITORING_TXT, 'class=mb-5', 'class=text-bg-info');

// php_errors_monitor_enabled
$form->addHelper(PHP_ERRORS_MONITOR_ENABLED_HELPER, 'php_errors_monitor_enabled');
$form->addCheckbox('php_errors_monitor_enabled', '', true, 'data-toggle=true, data-on-label=' . YES . ', data-off-label=' . NO . ', data-on-icon=' . ICON_CHECKMARK . ', data-off-icon=' . ICON_CANCEL . ', data-on-color=success-o');
$form->printCheckboxGroup('php_errors_monitor_enabled', PHP_ERRORS_MONITOR_ENABLED_TXT, false);

$form->startDependentFields('php_errors_monitor_enabled', 1);

$form->addOption('php_errors_monitor_level', '0', 'NOTICE + WARNING + FATAL|UNKNOWN ERROR');
$form->addOption('php_errors_monitor_level', '1', 'WARNING + FATAL|UNKNOWN ERROR');
$form->addOption('php_errors_monitor_level', '2', 'FATAL|UNKNOWN ERROR');

$form->addSelect('php_errors_monitor_level', 'ERROR LEVEL', 'data-slimselect=true, data-allow-deselect=false, data-show-search=false');

$form->addHelper(PHP_ERRORS_MONITOR_RECIPIENT_EMAILS_HELPER, 'php_errors_monitor_recipient_emails');
$form->addInput('text', 'php_errors_monitor_recipient_emails', '', EMAIL, 'required');
...
// lang
foreach (AVAILABLE_LANGUAGES as $lcode => $ltext) {
    $form->addOption('lang', $lcode, $ltext);
}
