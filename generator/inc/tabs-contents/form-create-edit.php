...
require_once GENERATOR_DIR . 'inc/default-generator-styles.php';
...
if (isset($_SESSION['generator'])) {
    ...
    if ($external_columns_count > 0) {
        foreach ($generator->external_columns as $key => $ext_col) {
            if ($ext_col['active'] === true && !empty($ext_col['relation']['intermediate_table'])) {
                ...
                $_SESSION['form-select-fields']['cu_ext_col_allow_in_forms-' . $key] = $ext_col['allow_in_forms'];
                $_SESSION['form-select-fields']['cu_ext_col_forms_fields-' . $key] = $ext_col['forms_fields'];
                $_SESSION['form-select-fields']['cu_ext_col_field_type-' . $key] = $ext_col['field_type'];
            }
        }
    }
    ...
    for ($i = 0; $i < $generator->columns_count; $i++) {
        ...
        $form_select_fields->startCol(12);
        $form_select_fields->addAddon('cu_special_file_dir_' . $column_name, '[ROOT_PATH]/', 'before');
        ...
        $form_select_fields->startCol(12);
        $form_select_fields->addAddon('cu_special_file_url_' . $column_name, '[ROOT_URL]/', 'before');
        ...
        $form_select_fields->startRow('form-group justify-content-start mb-3');
        $form_select_fields->startCol(12, 'sm', 'my-3');
        $form_select_fields->addAddon('cu_special_image_dir_' . $column_name, '[ROOT_PATH]/', 'before');
        $form_select_fields->addInput('text', 'cu_special_image_dir_' . $column_name, '', IMAGE_PATH . IMAGE_PATH_TIP, 'data-index=' . $i);
        $form_select_fields->endCol();

        $form_select_fields->startCol(12, 'sm', 'mb-3');
        $form_select_fields->addAddon('cu_special_image_url_' . $column_name, '[ROOT_URL]/', 'before');
        ...
        $form_select_fields->startDependentFields('cu_field_type_' . $column_name, 'date, datetime, month');
        ...
        if ($show_external === true) {
            ...
            foreach ($active_ext_cols as $key => $ext_col) {
                ...
                if (!empty($intermediate_table)) {
                    ...
                    $form_select_fields->addRadio('cu_ext_col_allow_in_forms-' . $key, YES, 1);
                    $form_select_fields->addRadio('cu_ext_col_allow_in_forms-' . $key, NO, 0);
                    ...
                    $form_select_fields->printRadioGroup('cu_ext_col_allow_in_forms-' . $key, $radio_label);

                    $form_select_fields->setCols(2, 10, 'md');
                    $form_select_fields->startDependentFields('cu_ext_col_allow_in_forms-' . $key, 1);
                    if (!isset($db)) {
                        $db = new DB(DEBUG);
                    }
                    ...
                    if (!empty($columns_count)) {
                        foreach ($columns as $field) {
                            $form_select_fields->addOption('cu_ext_col_forms_fields-' . $key . '[]', $field, $field);
                        }
                        $form_select_fields->addSelect('cu_ext_col_forms_fields-' . $key . '[]', VALUES_TO_DISPLAY, 'data-slimselect=true, multiple, data-maximum-selection-length=2, required');

                        $form_select_fields->addRadio('cu_ext_col_field_type-' . $key, SELECT_MULTIPLE, 'select-multiple');
                        $form_select_fields->addRadio('cu_ext_col_field_type-' . $key, CHECKBOXES, 'checkboxes');
                        $form_select_fields->printRadioGroup('cu_ext_col_field_type-' . $key, FIELD_TYPE);
                    }
                    $form_select_fields->endDependentFields();
                    $form_select_fields->endRowCol();
                    $form_select_fields->endCol();
    ...
    <script>
            ...
            echo str_replace(['<script>', '</script>'], '', (string) $script);
}
?>
</script>
