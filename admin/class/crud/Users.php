...
class Users extends Elements
{
    ...
    public $salt = array();
    public $email = array();
    public $activation_code = array();
    public $forgotten_password_code = array();
    public $forgotten_password_time = array();
    public $remember_code = array();
    public $created_on = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $columns = 'users.id, users.ip_address, users.username, users.password, users.salt, users.email, users.activation_code, users.forgotten_password_code, users.forgotten_password_time, users.remember_code, users.created_on, users.last_login, users.active, users.first_name, users.last_name, users.company, users.phone';
        $where = array();
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'users' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $this->forgotten_password_code[] = $row->forgotten_password_code;
                $this->forgotten_password_time[] = $row->forgotten_password_time;
                $this->remember_code[] = $row->remember_code;
