...
class ElementsFilters
{
    ...
    public function __construct($table, $filters, $join_query = '', $debug = false)
    {
        ...
        for ($i = 0; $i < $this->filters_count; $i++) {
            ...
            if (!isset($_SESSION['filters-list'][$filter_var])) {
                $_SESSION['filters-list'][$filter_var] = '**NOT_SET**';
            }

            if ($_SESSION['filters-list'][$filter_var] !== '**NOT_SET**') {
        ...
        if ($this->filters_count > 0) {
            ...
            for ($i = 0; $i < $this->filters_count; $i++) {
                ...
                if ($filter['ajax'] === true) {
                    ...
                    $form->addOption($field_var, '**NOT_SET**', '', '', 'label=' . SELECT_CONST . ', data-placeholder=true');
                    // if an ajax field is active we get the display value from a new query
                    if (isset($_SESSION['filters-list'][$field_var]) && $_SESSION['filters-list'][$field_var] !== '**NOT_SET**') {
                        // var_dump($filter);
                        $where_option = array();
                        $field_to_filter = $filter['field_to_filter'];
                        ...
                        if ($row = $db->selectRow($from, $columns, $where_option, $this->debug)) {
                            ...
                            $test_if_json = Utils::decodeJson($row->$field_value);
                            if (is_array($test_if_json)) {
                                ...
                                if (is_array($field_text)) {
                                    ...
                                    $opt = $this->getOptionTextAndValue($row->$field_text, $row->$field_value);
                        ...
                        if (!empty($values_count)) {
                                ...
                                while ($row = $db->fetch()) {
                                    ...
                                    if (is_array($test_if_json)) {
                                        ...
                                        if (is_array($field_text)) {
                                            ...
                                            $timestamp = $row->$field_value !== null ? strtotime($row->$field_value) : false;
                                            if ($timestamp) {
                            ...
                            for ($j = -1; $j < $values_count; $j++) {
                                if ($j == -1) {
                                    ...
                                    $form->addOption($field_var, $option_value[$j], $option_text[$j]);
                                }
                            }
                            ...
                            $form->addSelect($field_var, ucwords($filter['select_label']), 'class=form-select-sm, data-slimselect=true, data-allow-deselect=true, data-allow-deselect-option=true');
                        }
                    }
                }
            }
    ...
    public function getWhere($current_field = '')
    {
        ...
        foreach ($this->filters as $filter) {
            ...
            // if the field is not set or empty we set it to false
            if (!isset($_SESSION['filters-list'][$field_var]) || (empty($_SESSION['filters-list'][$field_var]) && $_SESSION['filters-list'][$field_var] !== (int) 0 && $_SESSION['filters-list'][$field_var] !== (string) '0')) {
                $_SESSION['filters-list'][$field_var] = '**NOT_SET**';
            }

            // echo $field_var . ' => ' . $_SESSION['filters-list'][$field_var] . '<br>';
            // echo $field_to_filter . ' => ' . $current_field . '<br><br>';

            if ($_SESSION['filters-list'][$field_var] !== '**NOT_SET**' && $field_to_filter != $current_field) {
                $datetime_field_types = explode(',', DATETIME_FIELD_TYPES);

                // if json values => $_SESSION['filters-list'][$field_var] = ~value~
                if (preg_match('`^~([^~]+)~$`', $_SESSION['filters-list'][$field_var], $out)) {
                    $qry_where[] = 'JSON_VALID(' . $field_to_filter . ') AND JSON_CONTAINS(' . $field_to_filter . ', \'["' . $out[1] . '"]\')';
                    ...
                    $real_value = $this->getRealValue($_SESSION['filters-list'][$field_var]);
                    if ($real_value !== false) {
                        $qry_where[$field_to_filter] = $real_value;
                    }
                }
            }
     ...
     * @param  string  $join_type         LEFT|INNER
     * @param  string  $left_equal_table
     * @param  string  $left_equal_field
     * @param  string  $right_equal_table
     * @param  string  $right_equal_field
     ...
     * @return string the final JOIN query for the current filter
     */
    ...
    public function showQuery()
    {
        ...
        return 'SELECT DISTINCT ' . $filter['fields'] . ' FROM ' . $filter['from'] . $qry_where;
    }
            ...
            if (isset($_POST['cancel_filters'])) {
                foreach ($_SESSION['filters-list'] as $key => $value) {
                    if (preg_match('`' . $list . '_filter_`', $key)) {
                        $_SESSION['filters-list'][$key] = '**NOT_SET**';
                    }
                }
            } else {
    ...
    /**
     * Set the appropriate texts & values for the filters options
     * @param mixed $text
     * @param mixed $value
     * @return array
     */
    private function getOptionTextAndValue($text, $value)
    {
        $option_text = $text;
        $option_value = $value;
        ...
        if (is_null($text)) {
            ...
            $option_text = '-';
            $option_value = '**EMPTY_STRING**';
        } elseif ($text === '0' || $text === 0) {
            $option_text = '0';
            $option_value = '0';
        }
