...
class Courses extends Elements
{
    ...
    public $course_duration = array();
    public $course_code = array();
    public $course_title = array();
    public $course_description = array();
    public $learning_outcome = array();
    public $fee_details = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $columns = 'courses.course_id, courses.course_duration, courses.course_code, courses.course_title, courses.course_description, courses.learning_outcome, courses.fee_details';
        $where = array();
        ...
        // order query
        $this->sorting = ElementsUtilities::getSorting($table, 'course_id', 'ASC');
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'courses' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $pk_concatenated_values = $row->course_id;
                ...
                $this->learning_outcome[] = $row->learning_outcome;
                $this->fee_details[] = $row->fee_details;
            }
        }
            ...
            if (!$this->is_single_view) {
                ...
                if (Secure::canUpdateRestricted($table)) {
                    ...
                    if (!empty($records_count)) {
                        while ($row = $db->fetch()) {
                            $this->update_record_authorized[$row->course_id] = true;
                        }
                    }
                }
            }
