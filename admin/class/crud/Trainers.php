...
class Trainers extends Elements
{
    ...
    public $trainer_id = array();
    public $trainer_username = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $columns = 'trainers.trainer_id, trainers.trainer_username, trainers.password, trainers.email, trainers.created_on, trainers.last_login, trainers.active, trainers.first_name, trainers.last_name, trainers.company, trainers.phone';
        $where = array();
        ...
        // order query
        $this->sorting = ElementsUtilities::getSorting($table, 'trainer_id', 'ASC');
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'trainers' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $pk_concatenated_values = $row->trainer_id;
                ...
                $this->trainer_username[] = $row->trainer_username;
            ...
            if (!$this->is_single_view) {
                ...
                if (Secure::canUpdateRestricted($table)) {
                    ...
                    if (!empty($records_count)) {
                        while ($row = $db->fetch()) {
                            $this->update_record_authorized[$row->trainer_id] = true;
                        }
                    }
                }
            }
