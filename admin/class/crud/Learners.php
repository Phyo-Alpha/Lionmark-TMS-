...
class Learners extends Elements
{
    ...
    public $learner_id = array();
    public $learner_username = array();
    ...
    public function __construct($element, $params = array())
    {
        ...
        $columns = 'learners.learner_id, learners.learner_username, learners.password, learners.email, learners.created_on, learners.last_login, learners.active, learners.first_name, learners.last_name, learners.company, learners.phone';
        $where = array();
        ...
        // order query
        $this->sorting = ElementsUtilities::getSorting($table, 'learner_id', 'ASC');
        ...
        $this->main_pdo_settings = array(
            'function' => 'select',
            'from'    => 'learners' . $active_filters_join_queries,
            'values'   => $columns,
            'where'    => $where,
        ...
        if (!empty($this->records_count)) {
            while ($row = $db->fetch()) {
                ...
                $pk_concatenated_values = $row->learner_id;
                ...
                $this->learner_id[] = $row->learner_id;
                $this->learner_username[] = $row->learner_username;
            ...
            if (!$this->is_single_view) {
                ...
                if (Secure::canUpdateRestricted($table)) {
                    ...
                    if (!empty($records_count)) {
                        while ($row = $db->fetch()) {
                            $this->update_record_authorized[$row->learner_id] = true;
                        }
                    }
                }
            }
