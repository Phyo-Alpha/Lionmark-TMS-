...
if ((Secure::canCreate('workshop_registrations') || Secure::canCreateRestricted('workshop_registrations')) && $_SERVER["REQUEST_METHOD"] == "POST") {
    /* =============================================
        delete if posted
    ============================================= */
    ...
    if (isset($_POST['records']) && is_array($_POST['records'])) {
        ...
        try {
            ...
            foreach ($_POST['records'] as $record) {
                ...
                $where = array('workshop_registrations.id' => $record_pk_value);
                if (!DEMO && !$db->delete('workshop_registrations', $where, DEBUG_DB_QUERIES)) {
