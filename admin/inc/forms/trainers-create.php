...
if ($_SERVER["REQUEST_METHOD"] == "POST" && Form::testToken('form-create-trainers') === true) {
    $validator = Form::validate('form-create-trainers', FORMVALIDATION_PHP_LANG);
    ...
    if ($validator->hasErrors()) {
        $_SESSION['errors']['form-create-trainers'] = $validator->getAllErrors();
    } else {
        ...
        try {
            ...
            if (DEMO !== true && $db->insert('trainers', $values, DEBUG_DB_QUERIES) === false) {
                $error = $db->error();
                ...
                if (!DEBUG_DB_QUERIES) {
                    ...
                    // reset form values
                    Form::clear('form-create-trainers');
...
$form = new Form('form-create-trainers', 'horizontal', 'novalidate');
$form->setAction(ADMIN_URL . 'trainers/create');
$form->startFieldset();
...
$form->endFieldset();
$form->addPlugin('pretty-checkbox', '#form-create-trainers');
$form->addPlugin('formvalidation', '#form-create-trainers', 'default', array('language' => FORMVALIDATION_JAVASCRIPT_LANG));
